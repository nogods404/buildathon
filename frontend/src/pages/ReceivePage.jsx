...
export default function ReceivePage() {
  ...
  const [handle, setHandle] = useState(localStorage.getItem('pingpay_claim_handle') || '')
  ...
  async function checkHandle() {
    ...
    try {
      ...
      if (err.message.includes('not found')) {
        setError('No wallet found for this handle. You may not have received any funds yet.')
      } else {
  ...
  async function handleWithdraw() {
    ...
    try {
      ...
      // Refresh wallet info
      const walletData = await api.getClaimWallet(handle.replace('@', '').toLowerCase())
      setWalletInfo(walletData.wallet)
      ...
      <div className="flex-1 px-4 py-4">
        {!verified ? (
          ...
          <div className="glass rounded-2xl p-6">
            ...
            <h2 className="text-xl font-bold text-center mb-2">Check Your Balance</h2>
          ...
          <div className="space-y-4">
            ...
            <div className="glass rounded-2xl p-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-dark-400">@{handle.replace('@', '')}</span>
                </div>
                <button
                  onClick={() => {
                    setVerified(false)
            ...
            {showWithdraw ? (
              <div className="glass rounded-2xl p-6">
                ...
                <div className="mb-4">
                  <label className="text-sm text-dark-400 mb-2 block">Destination Address</label>
                  ...
                  {connectedAddress && !withdrawAddress && (
                    <p className="text-xs text-dark-500 mt-2">
                      Leave empty to use connected wallet: {connectedAddress.slice(0, 8)}...
                    </p>
                  )}
                </div>
                ...
                <div className="mb-4 p-3 rounded-xl bg-dark-800/50">
                  ...
                  <p className="text-lg font-bold text-primary-400">{walletInfo?.balance || '0'} ETH</p>
                  <p className="text-xs text-dark-500 mt-1">(Full balance minus gas fees)</p>
                </div>
              ...
              <button
                ...
                disabled={!walletInfo?.balance || parseFloat(walletInfo?.balance) <= 0}
