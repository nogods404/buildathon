...
async function request(endpoint, options = {}) {
  ...
  if (!response.ok) {
    const errorMsg = data.details ? `${data.error}: ${data.details}` : (data.error || 'Request failed')
...
export const api = {
  ...
  createWallet: (handle) => request('/wallets/create', {
    method: 'POST',
    body: JSON.stringify({ handle })
  }),
  
  // Transfer endpoints
  parseCommand: (command) => request('/transfers/parse', {
    method: 'POST',
    body: JSON.stringify({ command })
  }),
  prepareTransfer: (data) => request('/transfers/prepare', {
    method: 'POST',
    body: JSON.stringify(data)
  }),
  confirmTransfer: (transferId, txHash, senderAddress) => request('/transfers/confirm', {
    method: 'POST',
    body: JSON.stringify({ transferId, txHash, senderAddress })
  }),
  estimateGas: (recipient, amount, senderAddress) => request('/transfers/estimate', {
    method: 'POST',
  ...
  verifyClaim: (token, handle) => request(`/claims/${token}/verify`, {
    method: 'POST',
    body: JSON.stringify({ handle })
  }),
  getClaimWallet: (handle) => request(`/claims/wallet/${handle}`),
  getPendingClaims: (handle) => request(`/claims/pending/${handle}`),
  withdraw: (handle, toAddress, amount, withdrawMax = false) => request('/claims/withdraw', {
    method: 'POST',
