...
router.post('/withdraw', async (req, res) => {
  ...
  if (!handle || !toAddress) {
    return res.status(400).json({ error: 'Handle and destination address are required' });
  }
  ...
  try {
    ...
    if (!wallet) {
      return res.status(404).json({ error: 'Wallet not found for this handle' });
    }
    ...
    if (withdrawMax) {
      ...
      if (!amount) {
        return res.status(400).json({ error: 'Amount is required when not withdrawing max' });
      }
    ...
    if (result.success) {
      ...
      res.status(500).json({ error: 'Withdrawal failed', details: result.error });
    }
  } catch (error) {
