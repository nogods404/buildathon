...
// Parse send command - now supports ETH
function parseCommand(command) {
  // Patterns: "send 0.01 eth to @alice", "send 0.01 to @bob", "pay @bob 0.05"
  const patterns = [
    /(?:send|pay)\s+(\d+(?:\.\d+)?)\s*(?:eth)?\s+to\s+@?(\w+)/i,
    /(?:send|pay)\s+@?(\w+)\s+(\d+(?:\.\d+)?)\s*(?:eth)?/i
  ];
...
// POST /api/transfers/parse - Parse a send command
router.post('/parse', (req, res) => {
  ...
  if (!parsed) {
    return res.status(400).json({ 
      error: 'Could not parse command. Try: "send 0.01 eth to @username"' 
    });
  }
...
router.post('/prepare', async (req, res) => {
  ...
  try {
    ...
    if (!recipientWallet) {
      ...
      recipientWallet = await createWallet(recipient, newWallet.address, newWallet.privateKey);
    }
    ...
    // Get gas estimate
    const gasEstimate = await estimateGas(senderAddress, recipientWallet.walletAddress, amount);
...
router.post('/confirm', async (req, res) => {
  ...
  if (!transferId || !txHash) {
    return res.status(400).json({ error: 'Transfer ID and transaction hash are required' });
  }
  ...
  try {
    ...
    if (transfer.status !== 'pending') {
      return res.status(400).json({ error: 'Transfer is not in pending state' });
    }
